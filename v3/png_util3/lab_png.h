...
int is_png(U8 *buf); //check if PNG signature is present
...
int get_png_chunks(simple_PNG_p out, U8* image_buffer, long offset); //extract from file all chunks in a png, to populate a struct simple_PNG
    //takes in file pointer and how to reach the IHDR chunk (see fseek parameters)
chunk_p get_chunk(U8 **buffer_position); //extract from file one chunk and populate a struct chunk
