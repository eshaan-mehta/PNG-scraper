...
int is_png(U8 *buf, size_t n) 
{
    ...
    return memcmp(png_signature, buf, n) == 0;
}
...
int get_png_data_IHDR(struct data_IHDR *out, FILE *fp, long offset, int whence)
{
    ...
    fseek(fp, offset + CHUNK_LEN_SIZE + CHUNK_TYPE_SIZE, whence);
    
    if (!fread(out, DATA_IHDR_SIZE, 1, fp)) return -1;
...
chunk_p get_chunk(FILE *fp)
{
    ...
    fread(out->type, CHUNK_TYPE_SIZE, 1, fp);
